...
1. Glob `prd/features/*/use-cases/UC-XXXX.md` (substituting the actual ID) to find the UC file. If not found:
...
3. Verify `prd/FEATURES.md` exists. If missing:
...
1. Glob `prd/features/*/use-cases/*.md` to find all UC files
...
Edit the UC file (`prd/features/FEAT-XXXX/use-cases/UC-XXXX.md`):
...
If `prd/features/FEAT-XXXX/ARCHITECTURE.md` exists:
...
3. New stubs must throw pending/not-implemented errors (per the reverse-engineering skill's step stub convention):
   ...
   - Body throws `Error('Step not implemented')` or equivalent
...
## Step 15: Update UC Status

1. If the UC file's current `status` is `backlog`, set it to `specified`.
2. Otherwise, only increment the `version` (already done in Step 7).
3. Update the corresponding row in `prd/features/FEAT-XXXX/USE-CASES.md`: change the Status column to match.

## Step 16: Report

Tell the user what was created:

- Scenarios appended to `prd/features/FEAT-XXXX/use-cases/UC-XXXX.md` (list SC-XXXX IDs and names)
